...
class ApiHandlerProducts:
    ...
    def _get_all_products(self):
        ...
        # Request Category URL
        url_products = requests.get(self.category_url + ".json")
        json_products = url_products.json()

        # Get Category number of products
        self.nb_products = int(json_products.get('count'))
        # Get Category number of pages
        self.nb_pages = ceil(self.nb_products / const.PRODUCTS_PER_PAGE)
    ...
    def get_random_products(self):
        ...
        if self.nb_prod_to_get > len(list_category_prods):
            ...
            str1 = """There are only {} products available in this
                       category.""".format(str(nb_prod_to_get))
